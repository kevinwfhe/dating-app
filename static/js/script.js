  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      if (json["redirect"]) {
        ...
        $(".toast-container").html(
          '<div data-delay="4000" class="toast fade"><div class="toast-header"><strong class="mr-auto"><i class="fa fa-globe"></i> Attention</strong><button type="button" class="ml-2 mb-1 close" data-dismiss="toast">&times;</button></div><div class="toast-body">' +
            json["message"] +
            "</div></div>"
        );
        $(".toast").toast("show", {
...
if ($("#page-ref").data("page-ref") == "chat") {
  ...
  $(".message-box-content").scrollTop(
    $(".message-box-content")[0].scrollHeight +
      parseInt($(".message-box-content").css("margin-bottom"))
  );
...
if (
  $("#page-ref").data("page-ref") == "view" ||
  $("#page-ref").data("page-ref") == "wink"
) {
...
if ($("#page-ref").data("page-ref") == "home") {
  ...
  const messages = [
    $(".right-quick-match-message .text"),
    $(".left-quick-match-message .text"),
  ];
  ...
  function reset_quick_messages() {
    $(messages).each(function () {
      $(this).css({
        ...
        color: "3px solid rgba(255, 255, 255, 0)",
      });
    });
  }
  ...
  $(function () {
    ...
    $(".draggable").draggable({
      ...
      drag: function (event, ui) {
        ...
        var right_distance =
          (-1 *
            ($("#droppable-right").offset().left -
              ($(this).offset().left + parseInt($(this).css("width"), 10))) +
            100) /
          100;
        var left_distance =
          (-1 * $(this).offset().left -
            $("#droppable-left").offset().left +
            100) /
          100;
        $(".right-quick-match-message .text").css({
          color: "rgba(255, 255, 255," + right_distance + ")",
          border: "3px solid rgba(255, 255, 255," + right_distance + ")",
        });
        $(".left-quick-match-message .text").css({
          color: "rgba(255, 255, 255," + left_distance + ")",
          border: "3px solid rgba(255, 255, 255," + left_distance + ")",
        });
      },
      scroll: false,
    });
    ...
    $("#droppable-right").droppable({
      ...
      drop: function (event, ui) {
        ...
        send_engagement(ui, "wink");
        ui.draggable.draggable("option", "revert", false);
        ui.draggable.css("display", "none");
        $("." + ui.draggable.attr("id") + ".draggable-view-profile").css(
          "display",
          "none"
        );
        reset_quick_messages();
...
if ($("#page-ref").data("page-ref") == "public_profile") {
  ...
  function readURL(input) {
    ...
    if (input.files && input.files[0]) {
      ...
      reader.onload = function (e) {
        $("#" + image_input_id)
          .parent()
          .find(".profile-delete-icon-container")
          .css("display", "inherit");
        $("#" + image_input_id)
          .parent()
          .find(".delete-hidden-input")
          .attr("checked", false);
        $("#" + image_input_id)
          .parent()
          .find(".profile-photo-image")
          .css("background-image", 'url("' + e.target.result + '")');
      };
      $(image_input_id)
        .siblings(".profile-delete-icon-container")
        .css("display", "block");
...
if ($("#page-ref").data("page-ref") == "member_profile") {
  $(window).bind("load", function () {
    ...
    $(".profile-img-draggable").draggable({
      axis: "x",
      containment: [
        window_width - draggable_profile_width,
        window_width - draggable_profile_width,
        0,
        0,
      ],
    ...
    $(window).resize(function () {
      ...
      $(".profile-img-draggable").draggable("option", "containment", [
        window_width - draggable_profile_width,
        window_width - draggable_profile_width,
        0,
        0,
...
if ($("#page-ref").data("page-ref") == "search") {
  ...
  var min_height_option = $(
    'select[name="height_min"] option[value="' + min_height + '"]'
  ).text();
  if (min_height_option) {
    $('.dropdown-toggle[data-id="height_min"] .filter-option-inner-inner').text(
      min_height_option
    );
  }
  var max_height_option = $(
    'select[name="height_max"] option[value="' + max_height + '"]'
  ).text();
  if (max_height_option) {
    $('.dropdown-toggle[data-id="height_max"] .filter-option-inner-inner').text(
      max_height_option
    );
  }
  var distance_option = $(
    'select[name="distance"] option[value="' + distance + '"]'
  ).text();
  if (distance_option) {
    distance_option;
    $('.dropdown-toggle[data-id="distance"] .filter-option-inner-inner').text(
      distance_option
    );
  }
  ...
  if (sexuality_arr) {
    ...
    for (i = 0; i < sexuality_arr.length; i++) {
      var sexuality_option = $(
        'select[name="sexuality"] option[value="' + sexuality_arr[i] + '"]'
      ).text();
    ...
    $('.dropdown-toggle[data-id="sexuality"] .filter-option-inner-inner').text(
      sexuality_title
    );
  }
  ...
  $(".search-form select").on("change", function () {
    if ($(this).val() == null) {
      $(this)
        .siblings(".dropdown-toggle")
        .removeClass("selected-search-option");
    } else {
